...
export default function DropdownActions({
  ...
  return (
    <div ref={dropdownRef} className={`relative flex justify-center`}>
      ...
      >
        {icon && !isUserMenu && <span className="w-5 h-5 me-1">{icon}</span>}
        {icon && isUserMenu && (
          <span className="w-10 h-10 me-1 flex items-center">{icon}</span>
        )}
      ...
      {menuItems.length > 0 && (
        <div
          id="dropdownMenu"
          className={`${className}-drop-menu z-40 ${
            isOpen ? "block" : "hidden"
          ...
          <ul className="flex flex-col gap-1.5 ">
            {menuItems.map((item, index) => (
              <li key={index}>
                ...
                >
                  {item.icon && (
                    <span className="icon w-4 h-4 me-2"> {item.icon}</span>
                  )}
