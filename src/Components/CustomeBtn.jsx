...
export default function CustomeBtn({
  ...
  return (
    <button
      ...
      className={"btn " + className + " " + size + (tooltip != null ? " tooltip_container" : "")+ (disabled == true ? " disabled" : "")}
      data-tooltip={tooltip && getTranslationText({ title: tooltip, lang: currentLanguage })}
      // Dynamically applies classes for styling
    >
      {isLoading ? (
        // Loading state: Show a spinner icon and a loading message
        <>
          <Iconloading className="w-4 h-4 text-primary " /> {/* Loading spinner */}
          {title &&
            <TranslationText
        ...
        <>
          {icon && !isLoading && <span className="icon_btn">{icon}</span>} {/* Optional icon */}
          {title &&
            <span>
              <TranslationText
                page={ResourcePage}
