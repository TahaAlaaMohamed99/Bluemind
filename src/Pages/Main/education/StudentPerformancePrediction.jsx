...
const StudentPerformancePrediction = () => {
  ...
  useEffect(() => {
    dispatch(setBreadcrumb(["Education", "Student Performance Prediction"]));
    return () => {
  ...
  const MediaMonitoringCards = ({ onCardClick }) => {
    const cards = [
      {
        id: "add",
        title: "Add New Student Performance Prediction",
  ...
  const handleCardClick = (cardId) => {
    if (cardId === "add") {
      navigate(`/studentPerformancePrediction/Add`);
    } else if (cardId === "plans") {
      navigate(`/studentPerformancePrediction/${cardId}`);
        ...
        <div className="mb-6 flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-gray-600 text-sm mt-1">
              Know what your audience talking about using our Education
            </p>
          </div>
        </div>
  ...
  const handleBack = () => {
    ...
    dispatch(setBreadcrumb(["Education", "House Prediction"]));
  };
  const renderContent = () => {
    switch (currentView) {
      case "add":
...
export default StudentPerformancePrediction;
